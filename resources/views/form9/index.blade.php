        ...
        <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
            <div class="px-6 py-4 mb-4 overflow-hidden bg-white shadow-sm dark:bg-gray-800 sm:rounded-lg">
                <div class="flex justify-between px-4 py-4 sm:px-6 sm:py-8">
                    ...
                    {{-- @can('create-forms') --}}
                        <a href={{ route('form9s.create') }} type="button" class="p-3 text-center text-gray-100 bg-green-600 rounded-md hover:bg-green-700">
                            ...
                            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 text-md">
                                ...
                                <td class="px-6 py-4 text-lg font-medium">
                                    @foreach ($form->items as $item)
                                        <li>{{ $item->name }} - مقدار: {{$item->pivot->quantity}}</li>
                                    @endforeach
                                </td>
                                <td scope="row" class="px-6 py-4 text-gray-900 whitespace-nowrap dark:text-white">
                                    {{ $form->requested_management}}
                                </td>
                                <td class="flex justify-end py-4">
                                    @can('view-items')
                                        <a href="{{ route('form9s.show', 1) }}" class="flex items-center justify-center p-2 font-medium text-center text-gray-200 bg-blue-600 rounded-md hover:bg-blue-700">
